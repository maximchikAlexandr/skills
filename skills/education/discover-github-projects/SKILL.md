...
1. Read `references/report-depth.md` before research and writing. For themed discovery also read `references/themes.md`. Before every registration apply `references/vidra-category-contract.md`; when a leaf is overfull also read `references/category-reindexing.md`. Use a short lowercase slash-separated subject path; do not classify by owner, date, language, or report format.
