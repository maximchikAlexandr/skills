...
1. Read `references/report-depth.md` before research and writing. For themed discovery also read `references/themes.md`. Before every registration read `references/category-reindexing.md`, inspect the live category tree and relevant sibling leaves, and pass its existing-leaf gate. Creating a leaf is the last resort: a merely narrower, synonymous, or cosmetically different label is not a new subject. Use a short lowercase slash-separated subject path; do not classify by owner, date, language, or report format.
2. Normalize the GitHub repository and check the shared registry with `vidra project seen OWNER/REPO`; one repository has one current project report. When the user asks to defer analysis, enqueue it with `vidra project queue-add OWNER/REPO`. Start only an explicitly requested item with `vidra project queue-begin OWNER/REPO --user-approved`. The Vidra UI presents project and video items in one chronological queue, while SQLite keeps their lifecycles separate. Use `vidra project remember` only for a screened repository that should not enter the queue.
...
Vidra stores project reports in real subject directories and returns `project_category_reindex_required` after a leaf directory grows beyond 15 projects. Reuse or safely broaden a related non-full leaf before creating a new one. During placement and reindexing, merge semantic duplicate leaves when one durable subject covers every member and the combined leaf stays within the limit; split an overfull leaf from evidence. Move reports with `vidra project move`, and verify with `vidra project category-tree` and `vidra doctor`. Never move registered files with raw filesystem commands.

Registration completes the matching project queue item automatically. If research or validation cannot complete, keep the item visible with `vidra project queue-fail OWNER/REPO --error REASON`; never publish a partial report as a successful analysis.
