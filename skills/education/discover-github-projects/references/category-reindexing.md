# Splitting overfull project categories

Read `references/vidra-category-contract.md` first. Use this reference only
when Vidra returns `project_category_reindex_required` for a leaf above the
15-record limit.
...
selection in React includes all descendant project categories. Re-run the
shared category contract's merge and duplicate checks after the split.
