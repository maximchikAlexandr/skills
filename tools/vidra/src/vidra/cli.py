...
from .domain import normalize_source, report_slug, require_transition
...
def view(row):
    ...
    out["report_url"] = f"reports/{Path(report).name}" if report else None
    return out
...
def complete(
    ...
    slug = report_slug(title)
    ids = "-".join(str(row["id"]) for row in rows)
    report_date = dt.datetime.now(dt.timezone.utc).date()
    name = f"{report_date}-{ids}-{slug}-{hashlib.sha256(source.read_bytes()).hexdigest()[:7]}.html"
    target = REPORTS / name
    ...
    with conn:
        for row in rows:
            conn.execute(
                "UPDATE videos SET status='analyzed',report_title=?,report_concept=?,report_path=?,completed_at=?,updated_at=?,error=NULL WHERE id=?",
                (title, concept, str(target), stamp, stamp, row["id"]),
            )
            conn.execute(
